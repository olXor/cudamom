...
void readTrainSet(std::string learnsetname, bool discard, size_t* numDiscards) {
	if (numDiscards != NULL) {
		numDiscards[0] = 0;
		numDiscards[1] = 0;
	}
	...
	while (getline(learnset, line)) {
		...
		for (int i = 1; i <= send && getline(datafile, dline); i++) {
			...
			dliness >> in;
			io.inputs[curinput] = in;
			curinput++;

			if (curinput >= NUM_INPUTS) {
...
float runSim(LayerCollection layers, bool train, float customStepFactor, bool print) {
	...
	for (size_t i = 0; i < trainset.size(); i++) {
		...
		error += fabs(newerror);
		if (print) {
			std::cout << "Output: " << h_output[0] << ", Correct: " << trainset[i].correctoutput << ", Error: " << newerror << std::endl;
		}
		if (train) {
...
#ifdef INITIAL_OUTPUT_THRESHOLD
	if (last)
