...
__host__ __device__ float transferFunction(float in) {
	...
	if (in / TRANSFER_WIDTH < -TRANSFER_FUNCTION_LIMIT)
		return 0;
	return TRANSFER_WIDTH*log(1.0f + exp(in / TRANSFER_WIDTH));
}
...
__global__ void convolve(ConvolutionMatrices* mat, ConvolutionParameters* pars) {
	...
	size_t numOutLocThreads = pars->forNBlockX;
	...
	for (size_t i = inNeuron; i < numInputNeurons; i += numInThreads) {
		for (size_t j = outNeuron; j < convSize; j += numOutThreads) {
			for (size_t k = outLoc; k < numOutputLocs; k += numOutLocThreads)
				inputs[i + j*numInputNeurons] = mat->inlayer[i + (k + j)*numInputNeurons];
		}
	}
	...
	for (size_t j = outNeuron; j < numOutputNeurons; j += numOutThreads) {
		for (size_t k = outLoc; k < numOutputLocs; k += numOutLocThreads) {
			if (inNeuron == 0)
				mat->outlayer[j + k*numOutputNeurons] = transferFunction(nodeStrengths[j*numInputNeurons]);
			if (inNeuron == 1 % numInThreads)
				mat->outTDs[j + k*numOutputNeurons] = transferDerivative(nodeStrengths[j*numInputNeurons]);
		}
	}
}
